...
**对话 (Conversation)**:
AVM 中最小的可调度执行单元。由系统提示词和用户提示词启动，通过 LLM 推理 → 指令执行 → 结果返回的循环运行，直到返回不含指令的结果时结束。可处于活跃、休眠、就绪三种调度状态。
_Avoid_: 会话、进程、agent

**子对话 (Child conversation)**:
通过 `create_cmd` 指令创建的新对话。子对话对 AVM 整体内存的目的负责，不对父对话负责。父对话进入休眠等待子对话结论。与 Linux 父子进程类似，父子松耦合。
_Avoid_: sub-agent
...
**服务 (Service)**:
通过 `register_service` 注册的已存在对话。其他对话可通过 `call_service` 调用并等待返回，或通过 `transfer_service` 移交控制权不等待返回。
...
`register_service`、`call_service`、`transfer_service`——对话间的调度关系管理，控制服务注册、调用和移交。
...
**Capability 模型**:
AVM 的安全基础。对话不知道内存地址就无法访问——不存在遍历根目录的操作。用户态对话看不到 `$MEM`，只知道被内核态告知的 workspace 路径和显式授权的特定节点路径。
